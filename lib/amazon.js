...
exports.search = function(itemPage, keywords, callback) {
  ...
  new OperationHelper(option).execute('ItemSearch', {
    ...
    if (typeof results.ItemSearchResponse.Items[0].Request[0].Errors !== 'undefined') { // if AWS return Error.
      callback(results.ItemSearchResponse.Items[0].Request[0].Errors[0].Error[0].Message);
      return;
    }
    callback(null, results.ItemSearchResponse.Items[0].Item.filter(function(item) {
      if (typeof item.ItemAttributes[0].EAN !== 'undefined') { // if item has isbn.
        return true;
      } else {
        return false;
      }
